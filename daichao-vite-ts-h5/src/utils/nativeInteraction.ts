...
// 跳转GooglePlay
export function openGooglePlay(appPkg) {
...
// 更换放款账户
export function changeAccount(productId, orderNo) {
  bridge.send("hncv_nu_aheogaAcmotcp", { productId: productId, orderNo: orderNo });
}
// 保存还款二维保
export function saveQrcode(imageUrl) {
  bridge.send("pvshamo_vad_eQcore", { imageUrl: imageUrl });
}
// 获取公参
export function getPublicParams(url, callback) {
...
// 解密数据
export function decryptData(data, callback) {
